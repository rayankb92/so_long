...
/*   Updated: 2022/11/25 18:03:23 by rferradi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mlx(t_data *img, t_position *map)
{
	...
	if (!img->mlx)
		return (0);
	img->mlx_win = mlx_new_window(img->mlx, map->x * 64, map->y * 64 + 64,
			"FOOTIX");
	if (!img->mlx_win)
		return (0);
	return (set_img(img));
...
void	split_img(t_data *img, t_position *map, int i, int j)
{
	...
	if (((j == 0) && (i != 0) && (i != map->y - 1)) || ((j == map->x - 1)
		...
		mlx_put_image_to_window(img->mlx, win, imtab[15], (j * 64), (i * 64));
	else if (i == map->y - 1 && j == 0)
		mlx_put_image_to_window(img->mlx, win, imtab[17], (j * 64), (i * 64));
	else if (j == 0 && i == 0)
		mlx_put_image_to_window(img->mlx, win, imtab[16], j * 64, i * 64);
	else if (j == map->x - 1 && i == map->y - 1)
		mlx_put_image_to_window(img->mlx, win, imtab[19], j * 64, i * 64);
	else if (j == map->x - 1 && i == 0)
		mlx_put_image_to_window(img->mlx, win, imtab[18], j * 64, i * 64);
	else
		mlx_put_image_to_window(img->mlx, win, imtab[10], (j * 64), (i * 64));
}
...
int	set_img(t_data *img)
{
	...
	static char	*ta[22] = {
		...
		WALL, P, GROUND, EXIT, BO, CO, COINHG, COINBG, COINHD, COINBD, GI, GID};
